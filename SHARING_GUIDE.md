...
Система шаринга позволяет пользователям делиться ссылками на конкретные события, которые автоматически откроют приложение на странице этого события **прямо в Telegram**, не выводя пользователя в браузер.
...
1. **"Поделиться"** (средняя кнопка) - **Telegram Mini App ссылки**:
   - **Остается в Telegram**: Использует формат `https://t.me/Fiesta_cursor_bot/My_Fiesta?startapp=event_ID`
   - **Автоматически выбирает лучший метод**: Telegram шаринг → системный шаринг → буфер обмена
   - **Генерирует красивый текст** с описанием события
   - **Telegram-нативные ссылки** которые открываются прямо в Mini App

2. **"Скопировать"** (первая кнопка) - умная кнопка:
   - **С настроенным ботом**: Копирует Telegram Mini App ссылку для открытия в Telegram
...
**Приоритетный формат (остается в Telegram):**
1. **Telegram Mini App**: `https://t.me/Fiesta_cursor_bot/My_Fiesta?startapp=event_123e4567-e89b-12d3-a456-426614174000`

**Fallback форматы:**
2. **URL параметры**: `https://your-app.com?event=123e4567-e89b-12d3-a456-426614174000`
3. **Telegram Bot**: `https://t.me/Fiesta_cursor_bot?start=event_123e4567-e89b-12d3-a456-426614174000`

> **🎯 Главное преимущество**: Пользователи остаются в Telegram и получают нативный опыт работы с Mini App!
