...
export const EventsList: React.FC<EventsListProps> = ({ 
  ...
  const getTabTitle = useCallback((tab: TabType): string => {
    switch (tab) {
      case 'my': return 'Актуальные мероприятия';
  ...
  const getEmptyStateMessage = useCallback((tab: TabType) => {
    switch (tab) {
      case 'my':
        return {
          icon: '📋',
          title: 'Нет актуальных мероприятий',
          subtitle: 'Создайте новое мероприятие или дождитесь запланированных!'
        };
      case 'archive':
        return {
          icon: '📦',
  ...
  const fetchEvents = useCallback(async (tab: TabType, page: number = 1, forceRefresh = false, silent = false) => {
    ...
    try {
      ...
      if (tab === 'archive') {
        ...
        // В "Актуальные мероприятия" показываем только активные и запланированные
        eventsData = eventsData.filter(event => !isEventCompleted(event));
      }
